...
Opened from a scene, the form starts filled in: the scene's title, a dropdown
of its URLs, and its performers, tags and studio ready to filter on.

## Scenes search

Filters on title, code (with EQUALS / NOT_EQUALS / INCLUDES / EXCLUDES), URL,
date (EQUALS / GREATER_THAN / LESS_THAN), performers, tags and studio, plus
sort field, direction and results per page.

Performers, tags and studio are matched by **stash-box id**, so they only work
for entries that carry one for the box being searched. Anything without an id
is listed but greyed out, with the reason shown, rather than silently ignored.

- **Performers start ticked.** Usually you want all of them.
- **Tags start unticked.** A scene carries many tags, and demanding all of them
  reliably matches nothing. Tick the one or two that actually narrow things.
